...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      try {
        if (!user.email) {
          console.error("[Flowinaja Auth] signIn failed: user.email is missing");
          return false;
        }
        ...
        if (dbUser) {
          ...
          if (!dbUser.isActive) {
            console.warn(`[Flowinaja Auth] signIn blocked: user ${normalizedEmail} is inactive`);
            return false;
          }
        ...
        console.error("[Flowinaja Auth Error in signIn callback]:", error);
        return false;
      }
    },
